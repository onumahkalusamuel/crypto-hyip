                ...
                <?php if (!empty($localData)) : ?>
                    <?php foreach ($localData as $index => $trans) : ?>
                        <tr>
                            ...
                            <td class="">
                                <a href="<?= $route->urlFor('admin-view-withdrawal', ['id' => $trans->ID]) ?>" class="btn btn-dark btn-theme-colored btn-sm btn-flat mb-5">VIEW</i></a><br />
                                <?php if ($trans->withdrawalStatus === "pending") : ?>
                                    <a href="<?=$route->urlFor('admin-approve-withdrawal', ['id'=> $trans->ID]);?>" class="btn btn-success btn-sm btn-flat mb-5" onclick="return confirmAction('Are you sure you want to approve this withdrawal? You acknowledge that you have paid this user the requested amount. Continue?')">APPROVE</i></a><br/>
                                    <a href="<?=$route->urlFor('admin-delete-withdrawal', ['id'=> $trans->ID]);?>" class="btn btn-dark btn-theme-colored2 btn-sm btn-flat mb-5" onclick="return deleteRecord()">DELETE</i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
